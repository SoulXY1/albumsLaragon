...
<body>
    ...
    <h1 class="container mt-5">Editar Album</h1>
    <form method="POST" action="/album/{{$album->id}}" id="formulario" class="container">
        @csrf
        @method('patch')

        <div class="form-group mt-3">
            <label for="title">Titulo del album</label>
            <input id="title" type="text" name="title" class="form-control" value="{{$album->title}}">

        </div>
        <div class="form-group mt-3">
            <label for="marca">Autor</label>
            <input id="author" type="text" name="author" class="form-control" value="{{$album->author}}">

        </div>
        <div class="form-group mt-3">
            <label for="year">Año de lanzamiento</label>
            <input id="year" type="year" name="year" class="form-control" value="{{$album->year}}">

        </div>
        <div class="form-group mt-3">
            <label for="price">Precio</label>
            <input id="price" type="price" name="price" class="form-control" value="{{$album->price}}">
        </div>
        ...
        <div class="form-group mt-3">
            ...
            <select id="genres" name="genres[]" multiple class="form-control">
                ...
                <option value="{{ $genre->id }}" @if($album->genres->contains($genre->id)) selected @endif>
                    {{ $genre->genreName }}
                </option>
                @endforeach
            </select>
